...
impl Component for ThemePreviewTile {
    ...
    fn preview(_window: &mut Window, cx: &mut App) -> Option<AnyElement> {
        ...
        let zerolimits = theme_registry.get("ZeroLimits");

        let themes_to_preview = vec![zerolimits.clone().ok()]
            .into_iter()
            .flatten()
            .collect::<Vec<_>>();
        ...
        Some(
            v_flex()
                ...
                .p_4()
                .children({
                    if let Some(zerolimits) = zerolimits.ok() {
                        vec![example_group(vec![single_example(
                            "Default",
                            div()
                                .w(px(240.))
                                .h(px(180.))
                                .child(ThemePreviewTile::new(zerolimits, 0.42))
                                .into_any_element(),
                        )])]
                    } else {
                        vec![]
